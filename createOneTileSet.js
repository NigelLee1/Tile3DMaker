...
function processData(pipes) {
  for (var i = pipes.length - 1; i >= 0; --i) {
    ...
    pipe.a.c3 = cesium_1.Cartesian3.fromRadians(pa.longitude, pa.latitude, h);
    pipe.b.c3 = cesium_1.Cartesian3.fromRadians(pb.longitude, pb.latitude, h);
  }
}
...
function getChild(pipes, index, root, tilesetDirectory, name, tileOptions) {
  ...
  for (let j = 0; j < fourPipes.length; j++) {
    if (fourPipes[j].length > 0) {
      ...
      if (index + 1 < exports.geometricErrorArr.length && geometryError !== 0)
        getChild(pipes0, index + 1, child, tilesetDirectory, name + "_" + j, tileOptions);
      children.push(child);
    }
  }
